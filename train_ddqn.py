...
def parse_args():
    ...
    parser.add_argument("--log-file",    type=str,   default="training_log_ddqn.csv")
    parser.add_argument("--buffer-dir",  type=str,   default="replay_buffer")
...
def main():
    ...
    if not args.no_resume:
        checkpoints = sorted(glob.glob(os.path.join(args.model_dir, "ddqn_ep*.pth")))
        if checkpoints:
    ...
    try:
        ...
        while episode <= args.episodes:
            ...
            if avg50 > best_avg50:
                best_avg50 = avg50
                agent.save(os.path.join(args.model_dir, "ddqn_best.pth"))
                print(f"  -> New best avg50: {best_avg50:.0f}")

            if episode % args.save_every == 0:
                path = os.path.join(args.model_dir, f"ddqn_ep{episode}.pth")
                agent.save(path)
                print(f"  -> Saved {path}")
